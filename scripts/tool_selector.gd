...
@onready var shop_menu: Control = $"../PauseUI/UpgradesMenu"
...
func _ready() -> void:
	...
	shop_button.tooltip_text = "Shop"
	shop_button.pressed.connect(shop_menu.open)
...
func update_unlocked_slots(_new_level: int) -> void:
	...
	shop_button.visible = PlayerProgress.is_tool_unlocked(Global.Tool.SONAR)

func _process(_delta: float) -> void:
	sonar_ammo_label.text = "∞" if Global.infinite_flares else str(PlayerProgress.sonar_ammo)
